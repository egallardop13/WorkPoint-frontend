...
interface BudgetRateData {
  totalBudget?: number[]
  totalActiveBudget?: number[]
  totalInactiveBudget?: number[]
}

function formatData(data: BudgetRateData, variant: string, isDarkMode: boolean) {
  const dataArray =
  ...
  const rateOfChange = (dataArray ?? []).map((value: number, index: number, array: number[]) => {
...
*/
interface BudgetRateOfChangeChartProps {
  budgets: BudgetRateData
  heading: string
  description: string
  variant: string
  metricType: string
...
export default function BudgetRateOfChangeChart({ budgets, heading, description, variant, metricType }: BudgetRateOfChangeChartProps) {
  const { theme } = useTheme()
  const isDarkMode = theme === 'dark'

  const budgetData = useMemo(() => formatData(budgets, variant, isDarkMode), [budgets, variant, isDarkMode])
  const budgetSum = useMemo(() => calculateNetChange(budgetData[0].data), [budgetData])
