...
function CustomColumnHeader(props: CustomColumnHeaderProps) {
  const { colDef } = props
...
export default function DepartmentsTable({ rowData }: DepartmentsTableProps) {
  ...
  const getRowClassName = useCallback(
    (params: { indexRelativeToCurrentPage: number }) => (params.indexRelativeToCurrentPage % 2 === 0 ? 'even' : 'odd'),
    []
  )
