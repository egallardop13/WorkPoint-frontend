...
interface CardComponentProps {
  ...
  id: string
  column: string
  handleDragStart: (e: React.DragEvent, card: CardData) => void
}

const Card = ({ title, id, column, handleDragStart }: CardComponentProps) => {
  return (
    <>
      ...
      <motion.div
        ...
        layoutId={id}
        draggable="true"
        onDragStart={(e) => handleDragStart(e as unknown as React.DragEvent, { title, id, column })}
        className="cursor-grab rounded border border-neutral-700 bg-neutral-800 p-3 active:cursor-grabbing"
      >
        <p className="truncate pr-3 text-sm text-neutral-100">{title}</p>
      </motion.div>
    </>
  )
}
