...
const PaginationButton = memo(function PaginationButton({ isActive, href, pageIdx, buttonIdx, pageCount }: PaginationButtonProps) {
  const activePage =
  ...
  let onClickPageIdx: number = 0

  let pageContent: number | string = ''
  if (pageCount > 7) {
    switch (buttonIdx) {
      case 0:
        pageContent = 1
